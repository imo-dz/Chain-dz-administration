...
public class AddClaimActivity extends AppCompatActivity {
    ...
    private TextInputLayout firstname_client_signup, email_clientSignUp,
            ...
            client_password_signUp, client_confirm_password_signUp,production_input;

    private AutoCompleteTextView wilayaAutoCompleteTextView,productionActv;
    private AppCompatButton registerButton;
    private LoadingDialog dialog;
    private int productionIdx;
    List<User>userList;
    @Override
    ...
    private void init()
    {
        ...
        production_input = findViewById(R.id.production_holder_register);

        productionActv = production_input.findViewById(R.id.client_production_register);
        userList = new ArrayList<>();
        List<String>productions = new ArrayList<>();
        db.collection(Constants.USERS_COLLECTION).whereEqualTo(Constants.ACCOUNT_TYPE,Constants.ACCOUNT_TYPE_PRODUCTION_MANGER).get().addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
            @Override
            public void onSuccess(QuerySnapshot queryDocumentSnapshots) {
                for(DocumentSnapshot documentSnapshot : queryDocumentSnapshots)
                {
                    ...
                    userList.add(user);
                    productions.add(user.getFirstName());
                }
            }
        });


        ArrayAdapter adapter = new ArrayAdapter(this, com.google.android.material.R.layout.support_simple_spinner_dropdown_item, productions);
        productionActv.setAdapter(adapter);
        productionActv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
        ...
        registerButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                dialog = new LoadingDialog(AddClaimActivity.this);
                if(user!=null)
                {
                    dialog.start();
                    createUser(user);
                ...
                .addOnSuccessListener(new OnSuccessListener<Void>() {
                    ...
                    public void onSuccess(Void unused) {
                        ...
                        new Handler().postDelayed(new Runnable() {
                            @Override
                            public void run() {
                                Intent intent = new Intent(AddClaimActivity.this, AdminHomeActivity.class);
    ...
    private User checkInputs()
    {
        ...
        production_input.setError(null);
        ...
        if (productionIdx==-1) {
            production_input.setError(getResources().getString(R.string.field_can_not_be_empty));
            cond=false;
        }
        if(cond)
        {
            User user = new User();
            user.setAccountType(Constants.ACCOUNT_TYPE_CLAIM_MANAGER);
            user.setFirstName(firstName);
            user.setLastName("");
            user.setEmail(email);
            user.setProductionId(userList.get(productionIdx).getId());
